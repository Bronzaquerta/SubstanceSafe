...
doctorList(List<Doctor> doctors){doctor_list=doctors;}
...
Future<void> _removeCredentials() async {
 ...
 static const String _keyDoctors = 'doctors';
   ...
   Future<void> saveDoctors(List<Doctor> doctors) async {
    ...
    List<String> doctorsJson = doctors.map((doctor) => jsonEncode(doctor.toJson())).toList();
   ...
   Future<List<Doctor>> getDoctors() async {
    ...
    if (doctorsJson != null) {
      return doctorsJson.map((doctorString) => Doctor.fromJson(jsonDecode(doctorString))).toList();
    } else {
      return [];
    }
  }
